...
function initPageVarsOnCreate(id) {
	...
	if (id === "Index") {
		...
		htmlClickEventHandlers(id, "menu");
	}else if (id === "NewComplaintSection") {
		htmlClickEventHandlers(id, "menu");
	}else if (id === "NewComplaintInfo") {
		htmlClickEventHandlers(id, "menu");
	}else if(id == "NewComplaintMap"){
		htmlClickEventHandlers(id, "menu");
	}
...
$(document).on("pageshow","#historyPage",function(){
	//Runs();
	var myParam = location.search.split('view=')[1];
	if(myParam == "mypost")
	{
		allPost("me");
	}
...
//#Complaint Selection
$(document).on('pagebeforeshow', '#NewComplaintSectionPage', function (event, data) {
	startBeforeShowVars(data);
	initPageVarsOnShow('NewComplaintSection');
});
$(document).on('pagecreate', '#NewComplaintSectionPage', function () {
	initPageVarsOnCreate('NewComplaintSection');
});
$(document).on('pageshow', '#NewComplaintSectionPage', function () {

	showcomplainttype();
	
});
//#Complaint Selection Info
$(document).on('pagebeforeshow', '#NewComplaintInfoPage', function (event, data) {
	startBeforeShowVars(data);
	initPageVarsOnShow('NewComplaintInfo');
});
$(document).on('pagecreate', '#NewComplaintInfoPage', function () {
	initPageVarsOnCreate('NewComplaintInfo');
});

$(document).on('pageshow', '#NewComplaintInfoPage', function () {
	showComplaintInfoPage();
});
//#Complaint Selection Info
$(document).on('pagebeforeshow', '#NewComplaintMapPage', function (event, data) {
	startBeforeShowVars(data);
	initPageVarsOnShow('NewComplaintMap');
});
$(document).on('pagecreate', '#NewComplaintMapPage', function () {
	initPageVarsOnCreate('NewComplaintMap');
});
$(document).on('pageshow', '#NewComplaintMapPage', function () {
	showMapView();
});
